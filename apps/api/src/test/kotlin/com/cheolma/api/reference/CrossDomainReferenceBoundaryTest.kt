 ...
 *
 * 두 가지 참조 경계만 좁게 보강한다.
 * 1. 신고 target 참조: 없는 대상 신고가 404 인 것은 `POST` 타입만 고정돼 있었다. POST_COMMENT/EVENT/
 *    EVENT_COMMENT 타입도 각각 자기 repo 로 대상 존재를 검증하므로 동일하게 404 인지 고정한다.
 * 2. 삭제된 parent 참조: 기존 테스트는 "처음부터 없는 id"만 404 로 고정한다. 실제 DELETE 로 제거한 게시글/행사에
...
class CrossDomainReferenceBoundaryTest(
    ...
    private fun saveDeletableEvent(): String {
    ...
    @Test
    fun `없는 대상 신고는 댓글 행사 행사댓글 타입도 404다`() {
        listOf("POST_COMMENT", "EVENT", "EVENT_COMMENT").forEach { type ->
    ...
    @Test
    fun `DELETE로 삭제한 행사에는 댓글을 작성하거나 참여할 수 없다`() {
        val id = saveDeletableEvent()
        mvc.delete("/api/events/$id") {
            headers { add("Authorization", "Bearer $token") }
        }.andExpect { status { isNoContent() } }

        mvc.post("/api/events/$id/comments") {
            headers { add("Authorization", "Bearer $token") }
            contentType = MediaType.APPLICATION_JSON
            content = """{"text":"삭제된 행사에 댓글"}"""
        }.andExpect { status { isNotFound() } }
