...
class PostControllerTest(
    ...
    private fun savePost(
        ...
        posts.save(
            Post(
                ...
                eventId = eventId, seq = seq, authorUserId = authorUserId,
            ),
        )
        return id
    }
    ...
    @Test
    fun `존재하는 eventId 는 201 로 저장된다`() {
        postPost("""{"text":"연결","eventId":"c1"}""").andExpect {
            status { isCreated() }
            jsonPath("$.eventId") { value("c1") }
        }
    }
    ...
    @Test
    fun `존재하지 않는 eventId 는 400`() {
        postPost("""{"text":"없음","eventId":"nope"}""").andExpect { status { isBadRequest() } }
    }

    @Test
    fun `blank eventId 는 null 로 저장된다`() {
        postPost("""{"text":"공백","eventId":"  "}""").andExpect {
            status { isCreated() }
            jsonPath("$.eventId") { value(null) }
        }
    }
    ...
    // ---- 수정 시 eventId 연결 검증 (행사 삭제 경합 방어로 lock 기반 검증) ----

    @Test
    fun `존재하는 eventId 로 수정하면 200 이고 연결된다`() {
        val id = savePost(authorUserId = 1)
        putPost(id, """{"text":"연결 수정","eventId":"c1"}""").andExpect {
            status { isOk() }
            jsonPath("$.eventId") { value("c1") }
        }
        assertThat(posts.findById(id).get().eventId).isEqualTo("c1")
    }

    @Test
    fun `없는 eventId 로 수정하면 400 이고 기존 eventId 와 본문이 유지된다`() {
        val id = savePost(authorUserId = 1, text = "원래 본문", eventId = "c1")
        putPost(id, """{"text":"바뀌면 안 됨","eventId":"nope"}""").andExpect { status { isBadRequest() } }
        val saved = posts.findById(id).get()
        assertThat(saved.eventId).isEqualTo("c1")
        assertThat(saved.text).isEqualTo("원래 본문")
    }

    @Test
    fun `eventId 가 null 이면 수정은 정상 동작하고 연결이 해제된다`() {
        val id = savePost(authorUserId = 1, eventId = "c1")
        putPost(id, """{"text":"연결 해제"}""").andExpect {
            status { isOk() }
            jsonPath("$.eventId") { value(null) }
        }
        assertThat(posts.findById(id).get().eventId).isNull()
    }
