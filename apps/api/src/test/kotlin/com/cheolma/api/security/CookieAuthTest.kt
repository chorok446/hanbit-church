...
class CookieAuthTest(
    ...
    fun `회원가입하면 httpOnly SameSite=Lax 인증 쿠키가 발급된다`() {
        mvc.post("/api/auth/signup") {
            ...
            content = """{"email":"cookie-signup@cheolma.com","password":"Passw0rd!","name":"쿠키"}"""
        }.andExpect {
            status { isCreated() }
    ...
    fun `인증 쿠키만으로 보호 API 를 호출할 수 있다`() {
        val token = jwt.issue(saveUser("cookie-me@cheolma.com"))
        mvc.get("/api/auth/me") { cookie(authCookie(token)) }
            .andExpect {
                status { isOk() }
                jsonPath("$.email") { value("cookie-me@cheolma.com") }
            }
    }
    ...
    fun `Authorization 헤더가 쿠키보다 우선한다`() {
        val token = jwt.issue(saveUser("cookie-priority@cheolma.com"))
    ...
    fun `쿠키 기반 로그아웃은 쿠키를 만료시키고 토큰을 무효화한다`() {
        val token = jwt.issue(saveUser("cookie-logout@cheolma.com"))
