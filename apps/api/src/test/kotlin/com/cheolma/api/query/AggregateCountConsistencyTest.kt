...
import com.cheolma.api.toElementList
 ...
 *
 * 게시글 `likes`/`comments`, 행사 `joined` 의 증감 자체는 각 ControllerTest 가 mutation 응답과 상세 조회에서
 ...
 * PostControllerTest·EventControllerTest·PR #78 이 이미 고정하므로 중복하지 않는다.)
 */
@SpringBootTest
...
class AggregateCountConsistencyTest(
    ...
    private fun saveJoinableEvent(): String {
    ...
    @Test
    fun `행사 참여자 수는 목록과 상세가 같은 값을 반환한다`() {
        ...
        assertThat(listField("/api/events", id, "joined")).isEqualTo(detailJoined)
    }
}
