...
import com.cheolma.api.report.Report
import com.cheolma.api.report.ReportRepository
import com.cheolma.api.report.ReportStatus
...
class AdminReportControllerTest(
    ...
    private fun saveEvent(): Event {
        val id = "admin-event-${UUID.randomUUID()}"
    ...
    fun `신고 목록은 최신순이고 대상 미리보기와 신고자 정보를 담는다`() {
        ...
        mvc.get("/api/admin/reports") {
            ...
            jsonPath("$.content[0].reporter.email") { value("test-user-2@cheolma.local") }
    ...
    fun `상태와 대상종류 필터가 동작하고 잘못된 필터는 400`() {
        ...
        val event = saveEvent()
        saveReport("POST", post.id, seq = 1)
        saveReport("EVENT", event.id, seq = 2, status = ReportStatus.DISMISSED.name)
        ...
        mvc.get("/api/admin/reports?status=DISMISSED&targetType=EVENT") {
            headers { add("Authorization", "Bearer $adminToken") }
        }.andExpect {
            status { isOk() }
            jsonPath("$.content.length()") { value(1) }
            jsonPath("$.content[0].target.href") { value("/events/${event.id}") }
        }
