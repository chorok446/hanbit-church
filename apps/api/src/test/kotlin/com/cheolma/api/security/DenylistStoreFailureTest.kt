...
class DenylistStoreFailureTest(
    ...
    fun `denylist store 장애 시 Bearer 인증 API 는 fail-closed 로 401이고 counter 가 증가한다`() {
        val user = repo.saveAndFlush(User(email = "denylist-down@cheolma.com", passwordHash = "h", name = "유저"))
    ...
    private fun failClosedCount(): Double =
        meterRegistry.find("cheolma.security.token_denylist.store_unavailable")
