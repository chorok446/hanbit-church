package com.cheolma.api.reference
 ...
 *
 * DELETE 는 hard delete 라 삭제된 게시글/행사은 어떤 조회 경로에도 다시 나타나면 안 된다. 기존 테스트는
 * 게시글 상세 404(작성자가 삭제하면 이후 조회는 404), 행사 mine 제외/row 제거만 고정한다. 다만 **목록/검색**
 * 재노출과 게시글 **mine** 재노출, 행사 **상세 404** 는 고정되지 않았다. soft delete 로 회귀하면 상세 404 만으로는
 * 목록/검색 재노출을 잡지 못하므로 대표 조회 경로 전반에서 비노출을 고정한다.
 * (삭제된 parent 하위 리소스 생성 불가는 PR #83, 행사 mine 제외/재삭제 404 는 EventControllerTest 가
 *  이미 고정하므로 중복하지 않는다.)
 */
@SpringBootTest
...
class DeletedResourceVisibilityTest(
    ...
    private fun saveDeletableEvent(marker: String): String {
        val id = "del-camp-${UUID.randomUUID()}"
        events.saveAndFlush(
            Event(
                id, "upcoming", "행사 $marker", "요약", "",
    ...
    @Test
    fun `삭제된 행사은 목록 검색에서 제외되고 상세는 404다`() {
        val marker = "delmark${UUID.randomUUID().toString().take(8)}"
        val id = saveDeletableEvent(marker)
        ...
        mvc.get("/api/events/$id").andExpect { status { isNotFound() } }
    }
}
