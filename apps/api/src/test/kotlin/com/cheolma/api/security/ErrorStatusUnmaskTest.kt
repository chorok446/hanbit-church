...
class ErrorStatusUnmaskTest(@param:Autowired val rest: TestRestTemplate) {
    ...
    fun `미인증 요청의 400 은 401 로 마스킹되지 않는다`() {
        ...
        val res = jsonPost(
            "/api/auth/signup",
            """{"email":"unmask400@cheolma.com","password":"short","name":"엑스"}""",
        )
    ...
    fun `미인증 요청의 409 는 401 로 마스킹되지 않는다`() {
        val email = "dup-${UUID.randomUUID()}@cheolma.com"
    ...
    fun `로그인 자격증명 오류는 401`() {
        val res = jsonPost(
            "/api/auth/login",
            """{"email":"nobody-${UUID.randomUUID()}@cheolma.com","password":"password1!"}""",
        )
