...
import com.cheolma.api.mapElements
 ...
 * 항목이 중복되거나 누락되지 않도록 대표 Page API 세 곳(게시글/행사/알림)에서 고정한다.
...
class PaginationContractTest(
    ...
    @Test
    fun `내 개설 행사 page는 page0과 page1 content가 겹치지 않고 전체를 덮는다`() {
        val ids = setOf(saveEvent(seq = 1), saveEvent(seq = 2), saveEvent(seq = 3))
        assertPageSplit("/api/events/mine/page", ids)
    }
