...
class EmailChangeConcurrencyTest(
    ...
    fun `두 사용자가 같은 이메일로 동시에 변경하면 하나만 200이고 다른 요청은 409`() {
        ...
        val first = users.saveAndFlush(
            User(email = "email-race-a-$suffix@cheolma.com", passwordHash = encoder.encode(password)!!, name = "A"),
        )
        val second = users.saveAndFlush(
            User(email = "email-race-b-$suffix@cheolma.com", passwordHash = encoder.encode(password)!!, name = "B"),
        )
        val target = "email-race-target-$suffix@cheolma.com"
