...
import com.hanbit.api.notification.Notification
 ...
 * 1. 게시글 검색 `eventOnly` 는 기본값이 false 다. PostSearchControllerTest 는 `eventOnly=true` 만 고정해
 *    필터 미지정/false 시 행사 비연결 게시글이 여전히 포함되는 기본 경계는 고정되지 않았다.
 ...
 *    섞이지 않는 경계는 고정되지 않았다.
 * (검색어/status/recruitState/availableOnly/날짜 범위 필터와 정렬은 Post/EventSearchControllerTest 및 PR #79 가
 *  이미 고정하므로 중복하지 않는다.)
 */
@SpringBootTest
...
class ListFilterBoundaryTest(
    ...
    private fun savePost(text: String, eventId: String?): String {
    ...
    @Test
    fun `게시글 검색 eventOnly 기본값은 행사 비연결 게시글도 포함한다`() {
        val keyword = "fltmark${UUID.randomUUID().toString().take(8)}"
        val linked = savePost(text = "$keyword 행사 글", eventId = "c1")
        val unlinked = savePost(text = "$keyword 일반 글", eventId = null)
