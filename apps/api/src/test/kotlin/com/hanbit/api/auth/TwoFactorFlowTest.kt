...
class TwoFactorFlowTest(
    ...
    @param:Autowired val jwt: com.hanbit.api.security.JwtService,
    ...
    fun `등록-로그인 챌린지-검증-해제 왕복이 동작한다`() {
        val user = saveUser("2fa-flow@hanbit.com")
        ...
        val loginRes = postJson("/api/auth/login", """{"email":"2fa-flow@hanbit.com","password":"$password"}""")
        ...
        val directLogin = postJson("/api/auth/login", """{"email":"2fa-flow@hanbit.com","password":"$password"}""")
            .andExpect { status { isOk() } }.andReturn().response
    ...
    fun `access 토큰은 챌린지 자리에, 챌린지 토큰은 access 자리에 쓸 수 없다`() {
        val user = saveUser("2fa-typ@hanbit.com")
