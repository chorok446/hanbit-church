...
@Transactional
class EventMyPagePaginationTest(
    ...
    private fun saveEvent(seq: Long, authorUserId: Long? = me): String {
    ...
    private fun addParticipant(eventId: String, userId: Long = me): String {
    ...
    @Test
    fun `비로그인은 401`() {
        mvc.get("/api/events/joined/page").andExpect { status { isUnauthorized() } }
        mvc.get("/api/events/mine/page").andExpect { status { isUnauthorized() } }
    }
    ...
    fun `page와 size를 검증한다`() {
        for (path in listOf("/api/events/joined/page", "/api/events/mine/page")) {
    ...
    @Test
    fun `내 참여 행사만 반환하고 joinedByMe true와 ownedByMe가 정확하다`() {
        val mineOwned = saveEvent(seq = 1, authorUserId = me) // 내가 개설하고 참여까지
        val otherOwned = saveEvent(seq = 2, authorUserId = 2L) // 남이 개설, 내가 참여
        val notJoined = saveEvent(seq = 3, authorUserId = 2L) // 참여 안 함
    ...
    fun `참여 취소 후 결과에서 제외된다`() {
        ...
        mvc.get("/api/events/joined/page") { headers { add("Authorization", "Bearer $token") } }
            .andExpect {
                status { isOk() }
    ...
    fun `기존 joined 배열 응답은 유지된다`() {
        val id = saveEvent(seq = 1, authorUserId = 2L)
        addParticipant(id)
        mvc.get("/api/events/joined") { headers { add("Authorization", "Bearer $token") } }
            .andExpect {
                status { isOk() }
                jsonPath("$") { isArray() }
    ...
    @Test
    fun `내 개설 행사만 최신순으로 반환하고 상태가 정확하다`() {
        val older = saveEvent(seq = 1, authorUserId = me)
        val newer = saveEvent(seq = 2, authorUserId = me)
        saveEvent(seq = 3, authorUserId = 2L) // 남의 행사
        saveEvent(seq = 4, authorUserId = null) // 레거시(authorUserId null)
        addParticipant(newer) // 내가 내 행사에 참여

        mvc.get("/api/events/mine/page") {
            headers { add("Authorization", "Bearer $token") }
            param("size", "9")
        }.andExpect {
            status { isOk() }
    ...
    @Test
    fun `개설 행사 page 분할`() {
        repeat(3) { saveEvent(seq = it.toLong(), authorUserId = me) }
        mvc.get("/api/events/mine/page") {
            headers { add("Authorization", "Bearer $token") }
    ...
    fun `기존 mine 배열 응답은 유지된다`() {
        saveEvent(seq = 1, authorUserId = me)
        mvc.get("/api/events/mine") { headers { add("Authorization", "Bearer $token") } }
            .andExpect {
                status { isOk() }
                jsonPath("$") { isArray() }
                jsonPath("$.length()") { value(1) }
