 ...
 * (게시글 댓글/행사 댓글/행사 참여)에서 **mutation 자체가 실패(검증/상태 충돌)한 경우** 리소스 소유자에게
 * 알림이 생성되지 않는지는 고정되지 않았다. notify 호출이 검증/상태 검사보다 앞으로 이동하는 리팩터링이 있으면
 * 실패 요청이 잘못 알림을 만들 수 있어, 각 지점의 실패 경로에서 알림 부재를 고정한다.
 * (실패 status 자체는 PR #75/기존 테스트가, 강제 퇴장 실패-무알림은 EventParticipantRemovalControllerTest 가
 *  이미 고정하므로 중복하지 않는다.)
 */
@SpringBootTest
...
class NotificationFailureSideEffectTest(
    ...
    private fun saveEvent(status: String): String {
        val id = "nf-camp-${UUID.randomUUID()}"
        events.saveAndFlush(
            Event(
                id, status, "행사 제목", "요약", "https://example.com/t.png",
    ...
    @Test
    fun `타인 행사에 빈 댓글 작성이 실패하면 개설자에게 알림이 없다`() {
        val eventId = saveEvent(status = "open")
    ...
    @Test
    fun `모집 중이 아닌 행사 참여가 실패하면 개설자에게 알림이 없다`() {
