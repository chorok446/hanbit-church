...
class LogoutDenylistTest(
    ...
    fun `로그아웃하면 같은 토큰으로 인증 API 호출 시 401`() {
        val token = jwt.issue(saveUser("logout-me@hanbit.com"))
    ...
    fun `로그아웃은 해당 토큰만 무효화하고 다른 사용자 토큰은 영향받지 않는다`() {
        val tokenA = jwt.issue(saveUser("logout-a@hanbit.com"))
        val tokenB = jwt.issue(saveUser("logout-b@hanbit.com"))
    ...
    fun `만료된 토큰 로그아웃은 401 - denylist 에 등록되지 않는다`() {
        ...
        val expired = JwtService(secret, -1000, 1209600000, "").issue(saveUser("logout-expired@hanbit.com"))
