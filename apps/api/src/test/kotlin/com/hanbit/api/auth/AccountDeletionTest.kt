...
import com.hanbit.api.event.EventParticipant
...
class AccountDeletionTest(
    ...
    @Test
    fun `비로그인 계정 탈퇴는 401`() {
        val user = saveUser("delete-unauthorized@hanbit.com")
    ...
    fun `탈퇴 요청은 비밀번호와 확인 문구를 검증하고 실패하면 계정을 유지한다`() {
        val email = "delete-validation@hanbit.com"
    ...
    fun `탈퇴하면 계정과 작성자만 익명화하고 관련 데이터와 딥링크를 유지한다`() {
        val originalEmail = "delete-success@hanbit.com"
