...
class ProfileSyncAuthorTest(
    ...
    fun `프로필 수정은 기존 작성물의 author snapshot 에 이름과 이미지를 전파한다`() {
        val user = users.saveAndFlush(
            User(email = "sync@hanbit.com", passwordHash = "hash", name = "기존이름", verified = true),
        )
        val userId = requireNotNull(user.id)
        val other = users.saveAndFlush(
            User(email = "other@hanbit.com", passwordHash = "hash", name = "남의이름", verified = false),
        )
    ...
    fun `프로필 이미지를 제거하면 작성물 snapshot 이미지도 제거된다`() {
        val user = users.saveAndFlush(
            User(
                email = "sync-clear@hanbit.com", passwordHash = "hash", name = "이름",
