...
class EventParticipantRemovalConcurrencyTest(
    ...
    private fun saveOpenEvent(capacity: Int, joined: Int): String {
        val id = "conc-rm-${UUID.randomUUID()}"
        eventRepo.saveAndFlush(
            Event(
                id, "open", "퇴장 동시성 행사", "요약", "https://x/y.png",
    ...
    private fun removeStatus(id: String, participantId: String): Int =
        mvc.delete("/api/events/$id/participants/$participantId") {
    ...
    fun `같은 participant에 대한 강제 퇴장과 본인 취소는 joined를 한 번만 감소시킨다`() {
        ...
        try {
            ...
            assertThat(participantRepo.countByEventId(id)).isEqualTo(0) // 최종 participant 없음
    ...
    fun `강제 퇴장과 재참여 동시 실행에도 500 없이 participant와 joined가 일치한다`() {
        ...
        try {
            ...
            val event = eventRepo.findById(id).get()
            val participantCount = participantRepo.countByEventId(id)
            assertThat(event.joined.toLong()).isEqualTo(participantCount) // 항상 일치
