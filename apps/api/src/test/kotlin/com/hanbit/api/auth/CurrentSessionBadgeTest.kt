...
class CurrentSessionBadgeTest(
    ...
    fun `현재 세션의 기록만 currentSession=true 이고 다른 로그인 기록은 false 다`() {
        saveUser("session-badge@hanbit.com")
        val first = login("session-badge@hanbit.com", chrome).cookieValue(AuthCookies.NAME)
        login("session-badge@hanbit.com", safari) // 다른 세션 하나 더
    ...
    fun `refresh 회전 후에도 같은 세션으로 표시된다`() {
        saveUser("session-refresh@hanbit.com")
        val loginRes = login("session-refresh@hanbit.com", chrome)
