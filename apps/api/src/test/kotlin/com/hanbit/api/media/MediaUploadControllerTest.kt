...
class MediaUploadControllerTest(
    ...
    fun `로그인 사용자는 이미지를 업로드하고 공개 URL로 조회할 수 있다`() {
        val user = users.saveAndFlush(User(email = "upload@hanbit.com", passwordHash = "h", name = "업로더"))
    ...
    fun `큰 이미지는 축소 저장되고 썸네일이 함께 생성된다`() {
        val user = users.saveAndFlush(User(email = "resize@hanbit.com", passwordHash = "h", name = "업로더"))
    ...
    fun `작은 이미지는 원본 크기를 유지하고 썸네일도 확대하지 않는다`() {
        val user = users.saveAndFlush(User(email = "small@hanbit.com", passwordHash = "h", name = "업로더"))
    ...
    fun `디코딩할 수 없는 업로드는 원본만 저장되고 썸네일은 만들지 않는다`() {
        val user = users.saveAndFlush(User(email = "no-thumb@hanbit.com", passwordHash = "h", name = "업로더"))
    ...
    @Test
    fun `지원하지 않는 형식은 400`() {
        val user = users.saveAndFlush(User(email = "bad-upload@hanbit.com", passwordHash = "h", name = "업로더"))
        mvc.perform(
            multipart("/api/media")
