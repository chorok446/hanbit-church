...
class MediaDocumentUploadTest(
    ...
    private fun admin(): User =
        users.saveAndFlush(
            User(email = "doc-admin-${System.nanoTime()}@hanbit.com", passwordHash = "h", name = "관리자")
    ...
    fun `일반 사용자의 문서 업로드는 403`() {
        val user = users.saveAndFlush(User(email = "doc-user@hanbit.com", passwordHash = "h", name = "성도"))
        mvc.perform(
            multipart("/api/media/document")
