...
class AuthRateLimitTest(
    ...
    fun `로그인은 IP당 limit 초과 시 429를 반환한다`() {
        val body = """{"email":"missing@hanbit.com","password":"wrong-password"}"""
        repeat(2) {
    ...
    fun `로그인은 IP가 달라도 계정당 limit 초과 시 429를 반환한다`() {
        ...
        val body = """{"email":"account-throttle-target@hanbit.com","password":"wrong-password"}"""
        repeat(2) { i ->
            mvc.post("/api/auth/login") {
                contentType = MediaType.APPLICATION_JSON
                content = body
    ...
    fun `회원가입은 IP당 limit 초과 시 429를 반환한다`() {
        ...
        val body = """{"email":"rate-limit-${UUID.randomUUID()}@hanbit.com","password":"password1!","name":"한도"}"""
        mvc.post("/api/auth/signup") {
            contentType = MediaType.APPLICATION_JSON
            content = body
        }.andExpect {
