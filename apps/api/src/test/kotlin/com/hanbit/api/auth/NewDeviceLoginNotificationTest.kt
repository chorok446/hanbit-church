...
class NewDeviceLoginNotificationTest(
    ...
    fun `첫 로그인은 무음이고 새 브라우저 로그인부터 알림이 온다`() {
        ...
        val user = saveUser("device-test@hanbit.com", password)
        val userId = requireNotNull(user.id)

        // 첫 로그인 — 접속 이력이 없으므로 알림 없음.
        login("device-test@hanbit.com", password, chromeMac).andExpect { status { isOk() } }
        ...
        login("device-test@hanbit.com", password, safariIphone).andExpect { status { isOk() } }
