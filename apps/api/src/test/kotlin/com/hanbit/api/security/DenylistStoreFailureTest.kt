...
class DenylistStoreFailureTest(
    ...
    class ThrowingDenylistStoreConfig {
        ...
        fun throwingDenylistStore(): TokenDenylistStore = object : TokenDenylistStore {
            ...
            override fun consume(tokenHash: String, ttlSeconds: Long): Boolean = throw RuntimeException("store down")
        }
    }

    @Test
    fun `denylist store 장애 시 Bearer 인증 API 는 fail-closed 로 401이고 counter 가 증가한다`() {
        val user = repo.saveAndFlush(User(email = "denylist-down@hanbit.com", passwordHash = "h", name = "유저"))
    ...
    private fun failClosedCount(): Double =
        meterRegistry.find("hanbit.security.token_denylist.store_unavailable")
