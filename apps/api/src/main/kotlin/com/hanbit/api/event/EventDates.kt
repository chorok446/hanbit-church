...
internal fun parseEventDate(value: String, fieldName: String): LocalDate {
    val trimmed = value.trim()
    if (trimmed.isEmpty()) {
        throw ResponseStatusException(HttpStatus.BAD_REQUEST, "$fieldName is required")
    }

    return parseSupportedEventDate(trimmed)
...
internal fun normalizeOptionalEventSearchDate(value: String?, fieldName: String): String? {
...
internal fun canonicalEventDateOrOriginal(value: String): String =
    parseSupportedEventDate(value.trim())?.toString() ?: value

enum class EventRecruitState(val value: String) {
...
internal data class EventRecruitment(
    val state: EventRecruitState,
...
internal fun Event.recruitmentOn(today: LocalDate): EventRecruitment {
    if (status == "closed") {
        return EventRecruitment(EventRecruitState.CLOSED, false, "모집완료")
    }
    if (status == "upcoming") {
        return EventRecruitment(EventRecruitState.BEFORE_RECRUIT, false, "모집 예정")
    }

    val start = parseSupportedEventDate(recruitStart.trim())
    val end = parseSupportedEventDate(recruitEnd.trim())
    if (start == null || end == null || start.isAfter(end)) {
        return EventRecruitment(EventRecruitState.ENDED, false, "모집완료", validDates = false)
    }
    if (today.isBefore(start)) {
        return EventRecruitment(EventRecruitState.BEFORE_RECRUIT, false, "모집 예정")
    }
    if (today.isAfter(end)) {
        return EventRecruitment(EventRecruitState.ENDED, false, "모집완료")
    }
    ...
    return EventRecruitment(EventRecruitState.RECRUITING, recruitable, label)
}

private fun parseSupportedEventDate(value: String): LocalDate? {
    val formatter = when {
