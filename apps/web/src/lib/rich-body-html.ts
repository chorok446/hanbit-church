...
/** 속성값으로 안전하게 삽입하기 위해 HTML 특수문자를 이스케이프한다. */
...
/** 이스케이프된 img src 를 원래 URL 로 되돌린다. */
...
export function splitRichBodyHtml(body: string): { html: string; images: string[] } {
  const images = Array.from(body.matchAll(IMG_SRC_RE), (match) =>
    unescapeHtmlAttr(match[1].trim()),
  ).filter(Boolean);
...
export function mergeRichBodyForEditor(text: string, images: string[]): string {
  ...
  const blocks = images
    .map((src) => `<p><img src="${escapeHtmlAttr(src)}" alt="본문 이미지" /></p>`)
    .join("");
