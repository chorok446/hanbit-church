 ...
 * --banner-bg 는 양 테마 모두 네이비라 크림(var(--on-banner)) 텍스트를 그대로 쓴다(home-visit 배너와 동일 관례).
 */
      ...
      <div className={`relative overflow-hidden ${className}`.trim()}>
        ...
        <div className="absolute inset-0 flex items-center justify-center" style={{ background: "rgba(22, 32, 58, 0.35)" }}>
          <span
            ...
            style={{ background: "rgba(var(--on-banner-rgb), 0.92)", color: "#1f2a44" }}
            aria-hidden
          >
    ...
    return (
      ...
      >
        <div className="min-w-0">
          ...
          <p
            className="mt-2.5 line-clamp-2 text-[17px] leading-snug text-[var(--on-banner)] sm:text-[19px]"
            style={{ fontFamily: "var(--font-display)", fontWeight: 600 }}
          >
            {meta.title}
          </p>
          {meta.scripture ? (
            <p className="mt-1.5 truncate text-[12.5px] text-[var(--on-banner)] opacity-65" style={{ fontFamily: "var(--font-display)" }}>
              {meta.scripture}
            </p>
          ) : null}
        </div>
        <p
          className="inline-flex items-center gap-1.5 self-start rounded-full border px-2.5 py-1 text-[11px] text-[var(--on-banner)] opacity-80"
          style={{ borderColor: "rgba(var(--on-banner-rgb), 0.3)" }}
        >
