        ...
        <div className="space-y-3 p-5" style={{ color: "var(--foreground)" }}>
          ...
          <div className="flex items-center justify-between text-[12px]" style={{ color: "rgba(var(--ink-rgb), 0.6)" }}>
            <span className="flex items-center gap-1.5">
              <Users size={12} aria-hidden /> {event.joined} / {event.capacity}명
            </span>
...
export function SavedEventsGrid({ page, onPageChange }: { page: number; onPageChange: (page: number) => void }) {
  const [removingId, setRemovingId] = useState<string | null>(null);
  const [actionError, setActionError] = useState("");

  const removeBookmark = async (eventId: string, reload: () => void) => {
  ...
  return (
    <PaginatedSection<Event>
      identityKey="saved-events"
      page={page}
      onPageChange={onPageChange}
      fetcher={fetchBookmarkedEventsPage}
