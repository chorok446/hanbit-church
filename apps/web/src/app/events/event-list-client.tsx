...
type SearchState = {
  ...
  response: EventSearchResponse | null;
...
export default function EventListClient({
  ...
  const urlState = useMemo<EventListUrlState>(() => ({
    query: searchParams.get("q") ?? "",
    filter: parseEventListFilter(searchParams.get("status")),
    recruitState: parseEventListRecruitState(searchParams.get("recruitState")),
    availableOnly: searchParams.get("availableOnly") === "true",
    ...readEventDateRangeFilters(searchParams),
    sort: parseEventListSort(searchParams.get("sort")),
    page: parsePageParam(searchParams.get("page")),
    view: parseEventListView(searchParams.get("view")),
  }), [searchParams]);

  const canonicalHref = buildEventsHref(urlState);
  const currentHref = searchParams.toString() ? `/events?${searchParams.toString()}` : "/events";
  ...
  const updateUrl = useCallback((changes: Partial<EventListUrlState>, replace = false) => {
    const href = buildEventsHref({ ...urlState, ...changes });
  ...
  useEffect(() => {
    ...
    apiGet<EventSearchResponse>(`/api/events/search?${params.toString()}`)
      .then((response) => {
    ...
    <>
      ...
      <PageShell ref={sectionRef} paddingClassName="relative min-h-screen overflow-hidden px-6 pb-20 pt-10" orb="right">
        ...
        <EventListFilters
          ...
          onClearDates={() => updateUrl({ ...EMPTY_EVENT_DATE_RANGE_FILTERS, page: 0 })}
        ...
        {currentState.status === "success" && response?.content.length === 0 ? (
          <ListEmptyState
            title={eventHasActiveFilters(urlState) ? "조건에 맞는 행사가 없어요." : "아직 등록된 행사가 없어요."}
            description={
              eventHasActiveFilters(urlState)
