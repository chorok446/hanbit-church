...
import { EVENT_TEMPLATES, type EventTemplate } from "@/data/event-templates";
import { PageShell } from "@/components/page-shell";

function draftHasContent(values: EventComposeValues): boolean {
  return (
...
export default function EventCreateClient() {
  ...
  const [values, setValues] = useState<EventComposeValues>(DEFAULT_EVENT_COMPOSE_VALUES);
  const [submitting, setSubmitting] = useState(false);
  const [fieldErrors, setFieldErrors] = useState<Partial<Record<EventComposeField, string>>>({});
  const confirm = useConfirm();

  const { draftSaved, clearDraft } = useEventComposeDraft(values, (draft) => {
  ...
  const submit = async () => {
    ...
    try {
      const created = await apiPost<Event>("/api/events", validation.payload);
