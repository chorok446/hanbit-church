...
function ProofItem({
  ...
  proof: EventProof;
  deleting: boolean;
  onDelete: (proof: EventProof) => void;
}) {
...
export function EventProofs({ event }: { event: Event }) {
  ...
  const requestIdentity = JSON.stringify([token, event.id, page, retryTick]);
  const [listState, setListState] = useState<{
    identity: string;
    status: "loading" | "success" | "error";
    response: EventProofsResponse | null;
  ...
  useEffect(() => {
    ...
    fetchEventProofs(event.id, page)
      .then((next) => {
    ...
    return guard.cancel;
  }, [event.id, page, token, requestIdentity]);
  ...
  const submit = async (formEvent: FormEvent<HTMLFormElement>) => {
    ...
    setMutationError("");
    try {
      await createEventProof(event.id, { text: composeText.trim(), images: composeImages });
  ...
  const removeProof = async (proof: EventProof) => {
    ...
    setMutationError("");
    try {
      await deleteEventProof(event.id, proof.id);
      if (getSessionId() !== requestToken) return;
      reload();
    } catch (error) {
      if (getSessionId() !== requestToken) return;
  ...
  // 서버 정책(EventProofService): 참여자만, 모집 시작(upcoming 이후)부터, 1인 1후기.
  const canCompose = !!token && event.joinedByMe && event.status !== "upcoming" && !proofedByMe;
  ...
  return (
    ...
    >
      <div className="flex items-center justify-between gap-3">
        <div>
          ...
          <p className="mt-1 text-[12px] opacity-60" style={{ color: "var(--foreground)" }}>
            참여 {event.joined.toLocaleString()}명 · 행사에 참여한 분은 함께한 순간을 사진과 함께 남겨보세요.
          </p>
        </div>
        <button
          type="button"
      ...
      {canCompose ? (
        ...
        >
          <label htmlFor="event-proof" className="text-[12px] font-medium" style={{ color: "var(--foreground)" }}>
            참여 후기 작성
          </label>
