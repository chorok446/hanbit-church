...
import {
  EventComposeForm,
  EventComposeSubmitButton,
} from "@/components/event-compose-form";
import { FallbackImage } from "@/components/fallback-image";
import {
  eventToComposeValues,
  statusMeta,
  type Event,
  type EventComposeField,
  type EventComposeValues,
  validateEventCompose,
} from "@/data/events";
import { PageShell } from "@/components/page-shell";

type LoadState =
  | { identity: string; kind: "loading" }
  | { identity: string; kind: "ready"; event: Event; initialValues: EventComposeValues }
...
function composeValuesChanged(a: EventComposeValues, b: EventComposeValues): boolean {
  return (
    a.title !== b.title ||
    a.summary !== b.summary ||
    a.body !== b.body ||
...
export default function EventEditClient({ id }: { id: string }) {
  ...
  const [values, setValues] = useState<EventComposeValues | null>(null);
  const [initialValues, setInitialValues] = useState<EventComposeValues | null>(null);
  const [saving, setSaving] = useState(false);
  const [fieldErrors, setFieldErrors] = useState<Partial<Record<EventComposeField, string>>>({});
    ...
    apiGet<Event>(`/api/events/${id}`)
      .then((event) => {
        ...
        if (!event.ownedByMe) {
          ...
          setLoadState({ identity, kind: "ready", event, initialValues: formValues });
        }
      })
      .catch((error) => {
        if (!isCurrent()) return;
  ...
  const clearFieldError = (field: EventComposeField) => {
  ...
  const save = async () => {
    ...
    const validation = validateEventCompose(values);
    if (!validation.ok) {
    ...
    try {
      const updated = await apiPut<Event>(`/api/events/${id}`, validation.payload);
