...
- **Feature flags**: DM REST API is off (`app.features.dm=false`, controller `@ConditionalOnProperty`; frontend DM UI is removed; `/ws/messages` WebSocket stays for notification push). Campaign creation can be limited to staff via `app.campaigns.staff-only-create` (default false to keep e2e green — enable in production).
