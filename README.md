...
철마제일교회 홈페이지. 커뮤니티 플랫폼 **다시,다(Dasida)** 를 포크해 교회 홈페이지로 리스킨한 프로젝트다. pnpm + Gradle 모노레포.

- 도메인 명칭: 캠페인 = "행사·사역", 피드 = "교제". 게시글 카테고리는 공지·주보·설교·나눔·기도.

## 주요 기능

- **게시글/카테고리**: 작성·수정·삭제, 이미지 업로드, 좋아요·북마크·댓글, 태그. 카테고리는 공지·주보·설교·나눔·기도(공지·주보는 관리자 전용 채널, 교제(/feed)는 나눔·기도만 노출, 설교는 전용 아카이브)
- **행사·사역(캠페인)**: 개설·모집 시작·참여/취소, 댓글, 참여자 목록
- **알림**: 타입별 필터 탭 + 안읽음 필터를 AND로 조합 조회
- **마이페이지**: 프로필 수정, 계정 설정(비밀번호/이메일 변경, 알림 설정), 접속 기록, 회원 탈퇴
- **검색/신고**: 게시글·행사·사역 통합 검색, 콘텐츠 신고
- **관리자**: 회원 승인(승인제 가입), 신고 처리, 콘텐츠 숨김, 새가족 등록 관리
...
| MySQL | `localhost:3306` (DB `cheolma`, user `cheolma`) |
...
- 종료: `Ctrl+C` 후 `docker compose -f compose.local.yml down` (DB 데이터는 volume `cheolma-mysql-data` 에 보존)
...
| `main` push | **CI 성공 후에만** `docker.io/<DOCKERHUB_USERNAME>/cheolma-api`, `cheolma-web` push (`sha-<shortsha>`, `main` tag) |
...
| `JWT_REFRESH_TTL_MS` | api | refresh token 만료(ms). 기본 1209600000(14일). httpOnly 쿠키(`cheolma_refresh`, `Path=/api/auth`)로만 전달, rotation 적용. |
