...
**철마제일교회 홈페이지** — a Korean church website (Cheolma First Church) forked from the **다시,다 (Dasida)** community platform and re-skinned as a church site. pnpm + Gradle monorepo. The `upstream` remote points at the original (dasida); only needed improvements are cherry-picked.
